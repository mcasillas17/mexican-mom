...
**Goal:** Publish a `0.1.2` release candidate in which all 24 skills have valid YAML, validation catches future frontmatter defects, safety routing is corrected, and cross-platform documentation matches the supported CLIs.
...
- Modify `README.md`: platform-specific lifecycle and invocation guidance.
- Modify `CHANGELOG.md`: add `0.1.2` and correct historical claims.
...
- Modify `VERSION` and four versioned manifests/catalogs: advance the package to `0.1.2`.
...
Add `0.1.2` with the YAML, validator, safety, and documentation fixes. Change the
...
- [ ] **Step 4: Bump all release metadata to `0.1.2`**

Set `VERSION`, both plugin manifests, and both versioned marketplace entries to `0.1.2`.
...
- Claude lists `mexican-mom@mcasillas17` version `0.1.2` as enabled.
- Copilot reports 24 installed skills and `/skills` shows no parse failures.
- Codex returns version `0.1.2` and 24 installed skills.
...
```bash
git push -u origin fix/release-hardening-v0.1.2
```
